...
def _merge_coco_splits(dataset_root: Path) -> tuple[Path, dict[str, Any], dict[str, Path]]:
    ...
    if not json_files:
        raise FileNotFoundError(f"No COCO JSON files found under {dataset_root}")
...
def download_detection_dataset(config: DetectionDownloadConfig) -> tuple[Path, dict[str, Any], dict[str, Path]]:
...
def load_or_download_detection_dataset(
    config: DetectionDownloadConfig, *, force_download: bool = False
    ...
    if not force_download and _has_coco_json(dataset_root):
        return _merge_coco_splits(dataset_root)
    return download_detection_dataset(config)
